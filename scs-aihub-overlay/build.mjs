...
const ua='Mozilla/5.0 (compatible; SCS-AIHub-Snapshot/2.0; +https://scs-aihub.vercel.app)';
...
function applyRewrites(text,base=ORIGIN+'/'){
 return text.replace(/(["'(=\s])((?:https?:\/\/scs-aihub\.vercel\.app)?\/[^"')\s,>]+\?[^"')\s,>]*)/g,(all,prefix,raw)=>{const u=cleanUrl(raw,base);if(!u)return all;const w=rewrites.get(u.href);return w?prefix+w:all});
}
for(const u of extractHtml(rootHtmlRaw,ORIGIN+'/'))enqueue(u);
while(queue.length&&seen.size<800){const u=queue.shift();try{const r=await get(u.href),ct=r.headers.get('content-type')||'',buf=Buffer.from(await r.arrayBuffer());const wp=webPathFor(u,ct);await writeBuf(diskFor(wp),buf);assetCount++;if(u.search)queryAssetCount++;
 if(/text\/html/i.test(ct)){pageCount++;for(const v of extractHtml(buf.toString('utf8'),u.href))enqueue(v)}
 else if(/text\/css/i.test(ct)){for(const v of extractCss(buf.toString('utf8'),u.href))enqueue(v)}
 else if(/(?:javascript|ecmascript)/i.test(ct)||/\.m?js$/i.test(u.pathname)){for(const v of extractJs(buf.toString('utf8'),u.href))enqueue(v)}
...
function inject(html){let x=html;if(!/<base\b/i.test(x))x=x.replace(/<head([^>]*)>/i,'<head$1><base href="/">');x=x.replace(/<\/head>/i,`<style id="scs-cost-style">${surveyCss.replace(/<\/style>/gi,'<\\/style>')}</style></head>`);x=x.replace(/<\/body>/i,`<div id="scs-cost-root"></div><script id="scs-cost-script">${surveyJs.replace(/<\/script>/gi,'<\\/script>')}</script></body>`);return x}
