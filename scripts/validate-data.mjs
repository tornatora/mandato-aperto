...
const allowedAreas = new Set(["Centrodestra", "Centrosinistra", "Centro", "Altro / non classificato"]);
...
assert(payload.meta?.methodologyVersion === "0.1.4", "versione metodologia inattesa");
assert(!/https?:/i.test(JSON.stringify(payload.meta?.sources ?? [])), "collegamento diretto presente nelle fonti pubblicate");
assert(!/\bM5S\b|MOVIMENTO\s+5\s+STELLE/i.test(JSON.stringify(payload)), "riferimento partitico non consentito nel dataset pubblico");
